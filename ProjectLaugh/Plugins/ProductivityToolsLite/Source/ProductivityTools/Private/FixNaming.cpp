...
void FFixNaming::FixNaming(TArray<FAssetData> SelectedAssets)
{
	...
	for (FAssetData AssetData : SelectedAssets)
	{
		...
		const FString PackagePath = FPackageName::GetLongPackagePath(AssetData.GetSoftObjectPath().GetAssetPath().ToString());
...
FString FFixNaming::GetPrefix(UObject* Asset)
{
	...
	else 
	{
		...
		else 
		{
			...
			{
				UE_LOG(LogProductivityToolsFixNaming, Warning, TEXT("Class name %s not defined for fix naming"), *ClassName);
				return "";
			}
		}
	}	
